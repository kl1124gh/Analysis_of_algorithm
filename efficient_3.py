...
def solve(S, T):
    ...
    fo = [[(0, 0)for _ in range(m + 1)] for _ in range(n + 1)]
    dp[0][0] = 0
    ...
    for i in range(n + 1):
        for j in range(m + 1):
            ...
            if i > 0 and dp[i][j] > dp[i - 1][j] + theta:
                dp[i][j], fo[i][j] = dp[i - 1][j] + theta, (i - 1, j)
            if j > 0 and dp[i][j] > dp[i][j - 1] + theta:
                dp[i][j], fo[i][j] = dp[i][j - 1] + theta, (i, j - 1)
            if i > 0 and j > 0 and dp[i][j] > dp[i - 1][j - 1] + alpha[s][t]:
                dp[i][j], fo[i][j] = dp[i - 1][j - 1] + alpha[s][t], (i - 1, j - 1)

    arr_S, arr_T = [], []
    i, j = n, m
    while i > 0 or j > 0:
        ni, nj = fo[i][j]
        arr_S.append('_' if ni == i else chars[S[i - 1]])
        arr_T.append('_' if nj == j else chars[T[j - 1]])
        i, j = ni, nj
    arr_S.reverse()
    arr_T.reverse()
    S = ''.join(arr_S)
